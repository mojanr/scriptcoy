...
CREATE TABLE `pesanan` (
  ...
  `tambahan` text,
  `idPetugas` varchar(7) DEFAULT NULL,
  PRIMARY KEY (`idPesanan`),
  KEY `idPetugas` (`idPetugas`),
  CONSTRAINT `pesanan_ibfk_1` FOREIGN KEY (`idPetugas`) REFERENCES `petugas` (`idPetugas`) ON DELETE SET NULL ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `pesanan` */

insert  into `pesanan`(`idPesanan`,`namaPemesan`,`noTelp`,`email`,`alamat`,`tglPesan`,`tglDeadline`,`totalHarga`,`status`,`finishing`,`orientasi`,`tambahan`,`idPetugas`) values ('WGM20180101001','asdf','asdf','asdf','asdf','0000-00-00','0000-00-00',6180000,'Sudah Dikonfirmasi','Gloss','Left Handed','ssss',NULL);
...
CREATE TABLE `petugas` (
  `idPetugas` varchar(7) NOT NULL,
  `username` varchar(15) DEFAULT NULL,
  `password` varchar(15) DEFAULT NULL,
  `bagian` enum('Administrator','Branding And Strategy','Luthier','Editor Audio Visual') DEFAULT NULL,
  `namaPetugas` varchar(30) DEFAULT NULL,
